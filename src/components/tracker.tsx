...
export function Tracker() {
  ...
  const [graphTemperatureData, setGraphTemperatureData] = useState<GraphData[]>(
    [{ id: 'Temperature', data: [] }],
  );
  const [graphClockData, setGraphClockData] = useState<GraphData[]>([
    { id: 'Clock', data: [] },
  ]);
  ...
  const toggleCapture = () => {
    if (!isCapturing) {
      ...
      setAverageTemperature(
        `${(tempSum / temperatureReadings.length).toFixed(2)}°C`,
      );
      setAverageClockSpeed(
        `${(clockSum / clockSpeedReadings.length).toFixed(2)} GHz`,
      );
    }
        ...
        <nav className="ml-auto flex items-center gap-4">
          <Button
            ...
            className={`text-white py-2 px-4 rounded ${
              isCapturing ? 'bg-red-500' : 'bg-gray-900'
            }`}
          >
      ...
      <Card className="mx-4">
        <CardHeader className="flex flex-row">
          <div className="grid gap-1.5">
            ...
            <CardDescription>
              Showcase new features in the dashboard.
            </CardDescription>
          </div>
